...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => [{
        type: "text",
        text: value.tree
          ? "已读取当前思维导图。要修改时用 mindmap_set 发送完整树：每个节点必须带 children 数组（叶节点为 []），不要改动或编造 theme 字段。"
          : "当前思维导图为空。"
      }]
    },
    execute(_args, exec) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: [
      ...
      "3. Do NOT invent theme. Send it only as a COMPLETE object containing palette:[\"#hex\",...] (e.g. copied from mindmap_get); a partial theme like {\"name\":\"dark\"} breaks rendering. Prefer omitting theme entirely.",
      "4. direction: 2 = both sides of the root (default, recommended), 1 = all left, 0 = all right.",
      "5. Valid JSON only: no comments, no trailing commas, no functions."
    ].join("\n"),
    parameters: {
      tree: {
        type: "json",
        required: true,
        description: "The complete MindElixirData object per the shape in the tool description: { nodeData: { id, topic, expanded, children: [] }, arrows: [], summaries: [], direction }. Omit theme."
      }
    },
    output: {
      schema: {
